...
public class MOFStateClassesGenerator {
	...
	public static CFG generateStateClasses(){
		...
		decNode=new DecisionNode("62#GGSC2:DecisionNode:C1","self.region.size() > 1");
		cond=new Condition("self.region.size() > 1");
		...
		atNode2=new AtomicNode("117#GSC6:else:GSC2","Line:118-Line122");
		R.insert(atNode1, decNode);
		...
		R.insert(atNode, compNode1);
		
		return R;
	}
	
	public static CFG generateClassesForStates(){
		AtomicNode atNode,atNode1,atNode2;
		CompositeNode compNode,compNode1;
		DecisionNode decNode;
		Condition cond;
		...
		decNode=new DecisionNode("214#GCFS5:DecisionNode:C1","s.isComposite == true");
		cond=new Condition("s.isComposite == true");
		decNode.setcondition(cond);
		...
		atNode2=new AtomicNode("231#GCFS8:else:GCFS5","Line:231-Line232");
		R.insert(atNode1, decNode);
	...
	public static CFG processCompositeState(){
		AtomicNode atNode,atNode1,atNode2;
		CompositeNode compNode,compNode1;
		DecisionNode decNode;
		Condition cond;
		...
		R.insert(atNode1, null);
		decNode=new DecisionNode("262#PCS2:DecisionNode:C1","self.region.size() > 1");
		cond=new Condition("self.region.size() > 1");
		decNode.setcondition(cond);
		R.insert(decNode, atNode1);
		atNode1=new AtomicNode("263#PCS3:then:PCS2","Line:263-Line266");
		R.insert(atNode1, decNode);
		...
		atNode2=new AtomicNode("289#PCS6:else:PCS2","Line:289-Line308");
		R.insert(atNode1, decNode);
...
//		R.insert(generateClassesForStates().getRootNode(),compNode1);//--------------f call
	...
	public static CFG createClassForState(){
		...
		decNode=new DecisionNode("395#CCS4:DecisionNode:C1","timeEventsExist");
		cond=new Condition("timeEventsExist");
		decNode.setcondition(cond);
		R.insert(decNode, compNode1);
		compNode=new CompositeNode("396#CCS5:then:CCS4"," self.generateTimeoutMethods()");
		...
		decNode=new DecisionNode("408#CCS13:DecisionNode:C2","isErrorState1");
		cond=new Condition("isErrorState");
		decNode.setcondition(cond);
		R.insert(decNode, compNode1);
		compNode=new CompositeNode("409#CCS14:then:CCS13"," self.generateGetErrorIdMethod(");
		R.insert(compNode, decNode);
	...
	public static CFG createCompositeStateClass(){
		AtomicNode atNode1;
		CompositeNode compNode,compNode1;
	...
	public static CFG createCompositeClassHavingParallelRegion(){
		AtomicNode atNode,atNode1;
		...
		decNode=new DecisionNode("1011#CCCHPR6:DecisionNode:C2","timeEventsExist");
		cond=new Condition("timeEventsExist");
		decNode.setcondition(cond);
		R.insert(decNode, atNode1);
		compNode=new CompositeNode("1012#CCCHPR7:then:CCCHPR6","  self.generateTimeoutMethods()");
		R.insert(compNode, decNode);
		R.insert(MOFStateClassHelper.generateTimeoutMethods().getRootNode(),compNode);//--------------f call
	...
	public static CFG generateCompositeStateClassWithParallelRegions(){
		AtomicNode atNode1;
		CompositeNode compNode;
	...
	public static CFG populateSignalEvents(){
		AtomicNode atNode1;
	...
	public static CFG populateCallEvents(){
		AtomicNode atNode1;
	...
	public static CFG populateChangeEvents(){
		AtomicNode atNode1;
	...
	public static CFG populateTimeEvents(){
		AtomicNode atNode1;
	...
	public static CFG generateTimeServiceCode(){
		AtomicNode atNode1;
	...
	public static CFG generateStateCodeForRegion(){
	...
	public static CFG generateSignalEvents(){
		AtomicNode atNode1;
		CompositeNode compNode;
