...
public class ExperimentDriverATL {
...
//		Search[] SA = new Search[] { new snt.oclsolver.search.AVM(), new snt.oclsolver.search.SSGA()  };
		for (Search sv : SA) {
			...
			BufferedWriter file = new BufferedWriter(new FileWriter(
					sv.getShortName()+"-MOFCoverageExperimentNewLP-" + startR + "-"
							+ numberOfRuns + ".txt"));
			BufferedWriter fileC = new BufferedWriter(new FileWriter(
					sv.getShortName()+"-MOFCoverageExperimentNewLPCond-" + startR + "-"
							+ numberOfRuns + ".txt"));
...
//							sv.setMaxIterations(totalIterations - currentIterationsConsumed);
				...
				file.flush();
				///////////
				fileC.write(sv.getShortName()); // Algo Name
				fileC.write("\tBreak"); // Approach Name
				fileC.write(" \t " + i); // No of Runs 
				fileC.write(" \t "+PredCov); // No Predicates solved
				fileC.write(" \t "+bd.doubleValue()); // Coverage Achieved
				fileC.write(" \t " + steps); // No of Iteration taken to achive coverage
				fileC.write(" \t " + currentIterationsConsumed); // No of Iteration taken to achive coverage				
				if(bd.intValue()==100)
				fileC.write(" \t true"); // Solution Found or Not
				else
			    fileC.write(" \t false"); // Solution Found or Not
				fileC.write(" \t " + elapsedTimeSec); // Time
				fileC.write(" \t " + totalElapsedTimeSec + "\r\n"); // Time
				fileC.write(" \t\n=======================================================\r\n");
				fileC.flush();
				////////
				k++;
